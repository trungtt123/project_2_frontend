...
function ProductTable(props) {
	...
	const [openModalProductBatchEdit, setOpenModalProductBatchEdit] = useState(false);
	const [openModalProductBatchProductCreate, setOpenModalProductBatchProductCreate] = useState(false);
	const [openModalProductBatchProductEdit, setOpenModalProductBatchProductEdit] = useState(false);
	...
	const handleDeleteProductBatch = () => {
		...
		if (window.confirm(`Bạn có chắc chắn xóa lô hàng mã ${productBatch.productBatchId}`) == false) {
	...
	const getListProducts = async () => {
		...
		var url =
			`https://localhost:7092/api/v1/product-batch?productBatchId=${productBatch?.productBatchId}`;
		var res = await axios
			.get(url, {
				headers: headers,
			});
	...
	const handleDeleteProductInBatch = async (id, productInfo) => {
		if (!window.confirm(`Bạn có chắc chắn muốn xóa sản phẩm này trong lô mã ${productBatch.productBatchId}`)) return;
	...
	return (
		...
		<div>
			...
			<div style={{ fontSize: 15, margin: 'auto', width: '95%', position: 'relative' }} className="mb-2">
				<InboxIcon />
				<span style={{
				...
				<span style={{ marginLeft: 250 }}>
					<ButtonGroup size="small" aria-label="small button group" >
						<Button color="primary" variant="contained" key="one" onClick={() => setOpenModalProductBatchEdit(true)}
			...
			<Collapse in={open} timeout="auto" unmountOnExit>
				...
				<div className="mt-3 mb-3" style={{ boxShadow: '2px 2px 2px 2px #AAA', borderRadius: '25px', margin: 'auto', width: '95%', padding: 10 }}>

					<Table style={{ margin: 'auto' }} size="small" aria-label="a dense table">
						<TableHead>
							<TableRow >
						...
						<TableBody>
							{listProductTable.map((product, index) => {
								var productInfo = productList?.find(o => o.productId === product.productId);
								var productDetail = {
									...product,
								...
								return <>
									...
									>
										...
										<TableCell align="right" component="th">{formatDateTime(product?.dateExpiry, 1)}</TableCell>
...
function InputInfo(props) {
	...
	const [openModalProductBatchCreate, setOpenModalProductBatchCreate] = useState(false);
	const { userList, isLoading } = useSelector((state) => state.user);
	const classes = useRowStyles();
	const receiverUser = userList?.find(o => o.userId === inputInfo.receiverUserId);
	const handleDeleteInputInfo = (inputInfoId) => {
		...
		if (window.confirm(`Bạn có chắc chắn xóa bản nhập hàng ${inputInfoId}`) == false) {
	...
	return <>
		<React.Fragment>
			<TableRow className={classes.root}>
				...
				<TableCell width="15%" align="right">{inputInfo?.shipper}</TableCell>
				<TableCell width="15%" align="right">
	...
	return (
		<div>
			...
			<Box
				...
				alignItems="flex-end"
			>
				<Button variant="contained" color="primary" sx={{ height: 40 }} onClick={() => setOpenModalInputInfo(true)}>
					Thêm trang
				</Button>
